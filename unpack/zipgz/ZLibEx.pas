...
//-hkm    FZStream   : TZStreamRec;
//-hkm    FBuffer    : Array [Word] of Byte;
...
begin
  ...
  if (zresult = Z_STREAM_END) {=hkm and (FZStream.avail_in > 0)} then
  begin
//-hkm    StreamPosition := StreamPosition - FZStream.avail_in;

//-hkm    FZStream.avail_in := 0;
//+hkm begin
    move(FZStream.next_in^,FBuffer,FZStream.avail_in);
    FZStream.next_in:=@FBuffer;
    FZStream.avail_in:=FZStream.avail_in+
      StreamRead(FBuffer[FZStream.avail_in],Sizeof(FBuffer)-FZStream.avail_in);
//+hkm end
  end;
