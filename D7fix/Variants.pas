...
var
  ...
  _src:PVarData;
begin
  //this procedure fixed by hkm dereference source
  _src:=@Source;
  while _src.VType = varByRef or varVariant do begin
    _src:=_src.VPointer;
  end;
  begin
      ...
      case _src.VType of
        ...
        varAny:
          if Assigned(VarDispProc) then
            VarDispProc(PVariant(LDestPtr), Variant(_src^), CallDesc, @Params);
      else
        // finally check to see if it is one of those custom types
        if FindCustomVariantType(_src.VType, LHandler) then
          LHandler.DispInvoke(LDestPtr, _src^, CallDesc, @Params)
        else
          VarInvalidOp;
      end;
    finally
