...
//-hkm    destructor  Destroy; override;
...
//-hkm destructor TGZDecompressionStream.Destroy;
//-hkm var
//-hkm   trailer: TGZTrailer;
//-hkm begin
//-hkm  inherited Destroy;

//-hkm  if StreamRead(trailer,SizeOf(TGZTrailer)) <> SizeOf(TGZTrailer) then
//-hkm  begin
//-hkm    raise EZDecompressionError.Create(zeStreamError);
//-hkm  end;

//-hkm  if (trailer.Crc <> FCrc) or (trailer.Size <> FSize) then
//-hkm  begin
//-hkm    raise EZDecompressionError.Create(zeDataError);
//-hkm  end;
//-hkm end;
...
//-hkm if StreamRead(trailer,SizeOf(TGZTrailer)) <> SizeOf(TGZTrailer) then
//+hkm begin
  if zresult<>Z_STREAM_END then exit;
  if FZStream.avail_in<SizeOf(TGZTrailer) then
//+hkm end
 begin
   raise EZDecompressionError.Create(zeStreamError);
 end;
...
//+hkm begin
 move(FZStream.next_in^,trailer,SizeOf(TGZTrailer));
 inc(pByte(FZStream.next_in),SizeOf(TGZTrailer));
 dec(FZStream.avail_in,SizeOf(TGZTrailer));
//+hkm end
