...
begin
  ...
  try
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            ArgPtr^[2] := ParamPtr^;
            //fix by hkm begin
            if (ArgType in [varDouble,varCurrency,varDate,varInt64]) then
            //fix by hkm end
            begin
    ...
    //add by hkm begin
    //fix call to method without params in form 'Obj.m_call()'
    if (InvKind = DISPATCH_METHOD) and (ArgCount = 1) and (CallDesc.ArgTypes[0]=varError) then begin
      DispParams.cArgs:=0;
    end;
    //add by hkm end
